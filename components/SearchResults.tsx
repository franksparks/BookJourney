                    ...
                    <TableRow key={index}>
                        <BookNavigationWrapper id={result.googleBooksId} key={index}>
                            <div className="flex flex-row justify-between">
                                <TableCell><img src={result.smallCover || "../default_cover.jpg"} alt={`{result.title}cover`} style={{ width: '75', height: '150px' }} /></TableCell>
                                <TableCell className="flex flex-col w-96"><b className="text-base">{`${capitalizeFirstLetter(result.title)}`}</b><div className="text-base">by {result.authors && result.authors.length > 0
