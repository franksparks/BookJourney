...
import AdvancedBookCard from "@/components/AdvancedBookCard";
    ...
    <Suspense>
      <main className="flex flex-col items-center bg-slate-200 p-8">
        <div className="w-2/3 flex flex-col items-center rounded-3xl shadow-sky-200 bg-sky-300">
          ...
          <div className="w-4/5 grid grid-cols-2 gap-4 p-8">
            {advancedResults.length !== 0 &&
              advancedResults.length !== 0 &&
              results.map((result: Book, index: any) => (
                <AdvancedBookCard book={result} key={index} />
              ))}
          </div>
          {advancedResults.length !== 0 && (
            <SearchPagination
